"""Fine-tune the 3.73M-parameter imitation policy with PPO."""
...
if __name__ == "__main__":
    ...
    model = PPO.load(
        ...
        target_kl=0.03,
        ent_coef=0.01,
        tensorboard_log="./cr_imitation_large_dir/",
    )
    ...
    callback = CheckpointCallback(
        ...
        save_path="./cr_imitation_large_dir/",
        name_prefix="cr",
    )
    try:
        model.learn(total_timesteps=15_000_000, callback=callback)
    finally:
        model.save("cr_imitation_large")
        env.close()
