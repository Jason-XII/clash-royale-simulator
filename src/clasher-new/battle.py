...
# - handle entity spawning, cards deployment
...
class BattleState:
    def __init__(self):
        ...
        # Now spawn Princess towers and King towers.
        # Probably have a tower down mechanic, will handle that later.
        self._spawn_entity(Entity(1, self.arena.RED_LEFT_TOWER, 1, 'King_PrincessTowers', []))
        self._spawn_entity(Entity(2, self.arena.RED_RIGHT_TOWER, 1, 'King_PrincessTowers', []))
        self._spawn_entity(Entity(3, self.arena.BLUE_LEFT_TOWER, 0, 'King_PrincessTowers', []))
        self._spawn_entity(Entity(4, self.arena.BLUE_RIGHT_TOWER, 0, 'King_PrincessTowers', []))
        self._spawn_entity(Entity(5, self.arena.RED_KING_TOWER, 1, 'KingTower', []))
        self._spawn_entity(Entity(6, self.arena.BLUE_KING_TOWER, 0, 'KingTower', []))
    ...
    def step(self, speed_factor: float = 1.0) -> None:
        ...
        if self.time > 240: regen = self.regen / 3
        elif self.time > 120: regen = self.regen / 2
        else: regen = self.regen
    ...
    def deploy_card(self, player_id: int, card_name: str, position: Position) -> bool:
        ...
        if not player.can_play_card(card_name): return False
