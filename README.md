...
而我的仓库则依赖于自建的完整模拟器逻辑，来弥补目前所有AI训练的经验收集瓶颈。这个模拟器读取准确的游戏数值，复现了原始游戏引擎的A*搜索寻路逻辑，卡牌交互基本准确。这数千行代码全由我一人编写，无AI Agent辅助。经过不断的优化，目前的模拟器性能可以做到0.5秒左右跑完180秒的对局，在我的M4芯片上实现大概396.8倍的加速。在一个一般般的CPU上，大概也能做到180倍相对于真实时间的加速。
